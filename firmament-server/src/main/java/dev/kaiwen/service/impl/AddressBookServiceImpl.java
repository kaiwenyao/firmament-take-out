...
public class AddressBookServiceImpl extends ServiceImpl<AddressBookMapper, AddressBook> implements
  ...
  public void updateAddress(AddressBook addressBook) {
    // 1. 验证参数并检查所有权
    validateAddressBookAndCheckOwnership(addressBook);
    Long userId = BaseContext.getCurrentId();

    // 2. 确保不能修改userId，防止越权
    addressBook.setUserId(userId);

    // 3. 使用 MyBatis Plus 的 updateById 方法更新地址
  ...
  public void setDefault(AddressBook addressBook) {
    // 1. 验证参数并检查所有权
    Long addressBookId = validateAddressBookAndCheckOwnership(addressBook);
    Long userId = BaseContext.getCurrentId();
  ...
  public AddressBook getByIdWithCheck(Long id) {
    // 验证地址存在并检查所有权
    return validateAddressOwnership(id);
  }
   ...
   * @return 地址ID
   */
  private Long validateAddressBookAndCheckOwnership(AddressBook addressBook) {
    if (addressBook == null) {
    ...
    // 验证地址是否存在且属于当前用户
    validateAddressOwnership(addressBookId);
   ...
   * @return 地址对象
   */
  private AddressBook validateAddressOwnership(Long id) {
    Long userId = BaseContext.getCurrentId();
